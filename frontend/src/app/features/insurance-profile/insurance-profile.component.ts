...
@Component({
  ...
  styleUrl: './insurance-profile.component.css',
})
export class InsuranceProfileComponent implements OnInit {
  ...
  readonly coverageTypeOptions = [
    { label: 'Basic Health', value: 'Basic Health' },
    { label: 'Comprehensive Health', value: 'Comprehensive Health' },
  ...
  loadProfile(): void {
    ...
    this.authService.getProfile().subscribe({
      ...
      error: (err) => {
        console.error('Error fetching insurance profile:', err);
        this.isLoading = false;
  ...
  loadInitialData(): void {
    ...
    if (tokenData) {
      this.userProfile.email = tokenData.sub || '';
    }
    this.userInitial = 'I';
  }
  ...
  mapProfileToData(profile: any): void {
    ...
    this.userProfile.coverageTypes = profile.coverageTypes || '';
    this.userProfile.claimProcessDescription =
      profile.claimProcessDescription || '';
    this.userInitial =
      this.userProfile.companyName.charAt(0).toUpperCase() || 'I';
  }
  ...
  saveProfile(form: NgForm): void {
    ...
    const updateData = {
      ...
      coverageTypes: this.userProfile.coverageTypes,
      claimProcessDescription: this.userProfile.claimProcessDescription,
    ...
    this.authService.updateProfile(updateData).subscribe({
      ...
      error: (err) => {
        ...
        console.error('Error updating insurance profile:', err);
